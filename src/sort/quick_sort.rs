...
fn quick_sort<T: PartialOrd + Copy>(array: &mut Vec<T>, index_left: usize, index_right: usize, comparisons:  &mut usize, swaps:  &mut usize, pb: &ProgressBar){
...
pub fn sort<T: PartialOrd + Copy>(mut array: Vec<T>) -> SortResult<T>{
    ...
    quick_sort(&mut array, index_left, index_right, &mut comparisons, &mut swaps, &pb);
